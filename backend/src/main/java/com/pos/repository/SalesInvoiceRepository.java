...
public interface SalesInvoiceRepository extends JpaRepository<SalesInvoice, Integer> {
    ...
    @Query("SELECT DISTINCT i FROM SalesInvoice i LEFT JOIN FETCH i.customer LEFT JOIN FETCH i.items it LEFT JOIN FETCH it.product p LEFT JOIN FETCH p.brand LEFT JOIN FETCH it.uom WHERE i.salesInvoiceId = :id")
    Optional<SalesInvoice> findByIdWithItems(@Param("id") Integer id);

    @Query("SELECT DISTINCT i FROM SalesInvoice i LEFT JOIN FETCH i.customer LEFT JOIN FETCH i.items it LEFT JOIN FETCH it.product p LEFT JOIN FETCH p.brand LEFT JOIN FETCH it.uom WHERE i.invoiceNumber = :invoiceNumber")
